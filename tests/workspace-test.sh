...
project_session=$(default_workspace_session "$TEST_DIR/project")
[[ $(tmux list-panes -t "=$project_session" | wc -l) -eq 4 ]] || fail 'expected four default panes'
[[ $(tmux display-message -p -t "=$project_session:" '#{pane_index}') -eq 0 ]] || fail 'expected editor focus'
for pane in $(tmux list-panes -t "=$project_session" -F '#{pane_id}'); do
...
tmux kill-session -t "=$(default_workspace_session "$TEST_DIR/editor-project")"
...
tmux kill-session -t "=$project_session"
...
editor=$(tmux list-panes -t "=$project_session" -F '#{pane_id}' | head -n 1)
...
[[ $(tmux list-panes -t "=$project_session" | wc -l) -eq 4 ]] || fail 'missing editor removed a pane'
tmux set-option -g default-command 'exec /bin/bash --noprofile --norc'
...
api_session=$(workspace_session api)
tmux has-session -t "=$api_session" || fail 'prefix match prevented exact session creation'
tmux kill-session -t "=$api_session"
tmux new-session -d -s api-server -n Unrelated
"$TMUXIFY" --detach --no-commands --file "$TEST_DIR/session.yml" > "$TEST_DIR/output"
tmux has-session -t "=$api_session" || fail 'multiple prefix matches prevented session creation'
tmux rename-window -t "=$api_session:" Preserved
"$TMUXIFY" --detach --file "$TEST_DIR/session.yml" > "$TEST_DIR/output"
[[ $(tmux list-windows -t "=$api_session" -F '#{window_name}') == Preserved ]] || fail 'exact reuse rebuilt a session'
...
geometry=$(tmux list-panes -t "=$(workspace_session geometry)" -F '#{pane_width} #{pane_height} #{pane_left} #{pane_top}')
printf '%s\n' "$geometry" | awk '
...
for scenario in final mixed automatic oversubscribed; do
  ...
  widths=$(tmux list-panes -t "=$(workspace_session "$scenario")" -F '#{pane_width}' | paste -sd, -)
...
if [[ "$1" == "${TMUXIFY_INTERRUPT_AT:-}" ]]; then
  ...
  if [[ -n "${TMUXIFY_RENAME_DURING_CREATION:-}" ]]; then
    "$TMUXIFY_REAL_TMUX" rename-session -t "=$TMUXIFY_INTERRUPTED_NAME" renamed-owned
    "$TMUXIFY_REAL_TMUX" new-session -d -s "$TMUXIFY_INTERRUPTED_NAME" -n Replacement
  fi
...
api_native=$(tmux display-message -p -t "=$api_session:" '#{session_id}')
grep -Fxq -- "$api_native" "$HOME/handoff" || fail 'attachment did not use the verified native session target'
socket_env=$(tmux display-message -p -t "$api_native" '#{socket_path},#{pid},0')
TMUX="$socket_env" "$TMUXIFY" --file "$TEST_DIR/session.yml" > "$TEST_DIR/output"
...
grep -Fxq -- "$api_native" "$HOME/handoff" || fail 'client switching did not use the verified native target'
...
interrupted_session=$(proposed_session "$TEST_DIR/interrupted.yml")
export TMUXIFY_INTERRUPTED_NAME="$interrupted_session"
for point in new-session set-window-option; do
  for signal in HUP INT TERM; do
    ...
    if tmux has-session -t "=$interrupted_session" 2>/dev/null; then fail "$signal at $point left an unfinished session"; fi
    ...
    tmux has-session -t "=$api_session" || fail 'interruption removed an existing session'
  done
done
...
if tmux has-session -t "=$interrupted_session" 2>/dev/null; then fail 'construction failure left an unfinished session'; fi
...
tmux has-session -t "=$interrupted_session" || fail 'attachment failure removed a completed workspace'
...
tmux kill-session -t "=$interrupted_session"
...
[[ $(tmux list-windows -t "=$interrupted_session" -F '#{window_name}') == Replacement ]] || fail 'rollback removed a replacement session with the same name'
...
vertical_session=$(workspace_session vertical)
heights=$(tmux list-panes -t "=$vertical_session" -F '#{pane_height}' | paste -sd, -)
[[ $heights == 48,32,15,16 ]] || fail "nested explicit-window vertical geometry is wrong: $heights"
[[ $(tmux display-message -p -t "=$vertical_session:" '#{pane_index}') -eq 3 ]] || fail 'geometry changes lost explicit-window focus'
...
if tmux has-session -t "=$(proposed_session "$TEST_DIR/small.yml")" 2>/dev/null; then fail 'impossible layout left an unfinished session'; fi
